<template>
  <div class="days-list">
    ...
    <p class="caption" v-if="checkEmpty(currentCity)">
      To see the forecast please enter the city in home tab!
    </p>
    <h2 class="title" v-if="!checkEmpty(currentCity)">
      {{ this.currentCity.cityName }},
      <span class="subtitle"> {{ this.currentCity.country }} </span>
    </h2>
    ...
    <div class="content" v-if="dataLoaded">
      <ul class="list">
        <li v-for="(item, index) in daysList" class="list__item" :key="item.dt">
          <i
            class="bx list__item__icon"
            :class="getIcon(item.weather[0].main)"
          ></i>
          <p class="list__item__day">
            {{ getWeekDay(index) }},
            <span class="list__item__date"> {{ getDate(index) }}</span>
          </p>
    ...
    async getData(city) {
      ...
      try {
        ...
        let response = await fetch(
          `https://api.openweathermap.org/data/2.5/onecall?lat=${this.currentCity.lat}&lon=${this.currentCity.lon}&exclude=current,minutely,hourly&appid=${this.API_KEY}`
        );
